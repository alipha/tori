...
namespace Protocol
{
    public class Packet // overhead is about 406 bytes. max udp packet is 1280 (or 1500?). 9% - 32% overhead (8% - 28%)
    {
        ...
        public byte[] Id { get; set; }  // on the outer layer only (16 bytes), null otherwise. Each node does a keyed hash of the Id value to get the next Id value
        ...
        public short EncryptedLen { get; set; }  // 2 bytes
        ...
        public Packet Layer { get; set; }   // Layer or Content is null

        public PacketContent Content { get; set; }

        public byte[] Mac { get; set; }     // 32 bytes (does this need to be on every layer to prevent malicious nodes from changing it to DoS clients or other nodes?)
    }
}
